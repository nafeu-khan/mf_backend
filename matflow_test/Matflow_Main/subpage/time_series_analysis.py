...
def time_series_analysis(file):
    ...
    print(f"type = {type (min_date_range)}   {min_date_range}  {dt}")
    max_date_range =datetime.strptime(dt, detected_format)
    print(f"type = {type(max_date_range)}  {max_date_range}")
...
#     # data = data.set_index('date')
#     # ----------datetime
#     date_string =  str(data["date"][2].date())
#     formats = [
#         "%Y-%m-%d %H:%M:%S",
#         "%Y-%m-%d",  # Year-Month-Day
#         "%Y/%m/%d",  # Year/Month/Day
...
#     filtered_data = data.loc[selected_start:selected_end]
#
#     # filtered_data = data.loc[(data['date'] >= min_date_range) & (data['date'] <= max_date_range)]
#
#     if isinstance(filtered_data, pd.DataFrame):
#         fig = go.Figure()
#         # Add the line plot
